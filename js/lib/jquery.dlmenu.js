...
;( function( $, window, undefined ) {
	...
	$.DLMenu.prototype = {
		...
		_config : function() {
			...
			this.$topTitle = this.$el.find('h2').html();
		},
		_initEvents : function() {
			...
			this.$menuitems.on( 'click.dlmenu', function( event ) {
				...
				if( $submenu.length > 0 ) {
					var upLevel = $item.children().first().clone();
					self.$menu.find('h2').html(upLevel);
					...
					if( !$item.hasClass('showMenu') && self.supportAnimations) {
			...
			this.$back.on( 'click.dlmenu', function( event ) {
				...
				if( $subview.is( 'li' ) ) {
					var upLevel = $subview.children().first().clone();
					self.$menu.find('h2').html(upLevel);
					$subview.addClass( 'dl-subviewopen' );
				}else{
					self.$menu.find('h2').html(self.$topTitle);
				}
